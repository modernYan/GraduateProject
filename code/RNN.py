...
with tf.variable_scope('main'):
    ...
    main_RNN_cell = tf.contrib.rnn.BasicRNNCell(hidden_size, reuse=tf.get_variable_scope().reuse)
    main_cell = tf.contrib.rnn.DropoutWrapper(main_RNN_cell, output_keep_prob=0.8)
...
def train_model():
    ...
    准备好文件夹
    """
    logdir = "tensorboard/RNN_{}/".format(int(time.time()))
    modeldir = 'models/RNN_models/'
    import shutil
    import os
    try:
    ...
    for i in range(iterations):
        ...
        if (i != 0 and i % 5 == 0) or i == 1:
            ...
            if accuracy_temp > max_accuracy:
                save_path = saver.save(sess, "models/RNN_models/{}.ckpt".format(accuracy_temp), global_step=i)
...
def test_model():
    ...
    batch_size = 7000
    print('START TEST')
    model_file = tf.train.latest_checkpoint('models/RNN_models/')
    ...
    print('Test Result: {}'.format(accuracy_temp))
